 ...
 * 
 * config.properties文件到底放在哪里合适？
 * 	1. 唯一的标准做法是：一定要记住用完整的路径，但完整的路径不是硬编码，而是运算出来的。可读可写配置文件。
 * 	2. 我们也可以通过classLoader来加载资源文件，但是这种方法不能代替1。只可读配置文件。
 * 	以后学习的框架，Spring，Struts 都是使用的第2种方法，都将配置文件放在ClassPath路径下。
 * */
